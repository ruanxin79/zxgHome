    ...
    <div class="cube-popup-container">
        <div class="cube-popup-content">
            <div class="cube-action-sheet-panel cube-safe-area-pb" style="">
                <h1 class="cube-action-sheet-title border-bottom-1px">选服务</h1>
                <div class="cube-content">
                    <div class="cube-action-sheet-content">
                    <h2 class="label servlabel"><span v-for="(i,ind) in servlabel" :key="ind">{{i.full_name}}</span></h2>
                        <ul class="cube-action-sheet-list">
                        <li class="cube-action-sheet-item border-bottom-1px cube-foo">
                            <div id="servModal">
                                <p v-for="(item,index) in data" :key="index" @click="changeLabel(item , index)" :class="!item.active ? '' : 'active'"><span class="name">{{item.abbreviation}}</span><span class="chi">{{item.package_price}}</span></p>
                            </div>
                        </li>
...
export default {
  ...
  methods: {
    handelClick (name) {
        if(name) {
            this.$emit('showServModal', this.servlabel)
        }  
    },
    changeLabel (item,index) {
        ...
        if(item.active) {
            ...
            this.servlabel.map((i,v)=>{
                if(i.full_name == item.full_name) {
                    this.servlabel.splice(v,1);
                }
            })
        }
